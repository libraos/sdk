...
Tracks Python surface added since `1.0.0`. Targets a `1.1.0` minor cut once the partner-prefix wave settles. The OpenAPI spec advanced through `1.0.0-alpha.3` → `1.0.0-alpha.4` → `1.0.0-alpha.5` to declare the new server endpoints; the matching server build is `ghcr.io/meganovaai/nova-os:v0.1.7` (and `:latest`).
...
*Since [v0.1.7](https://github.com/MeganovaAI/nova-os/releases/tag/v0.1.7) (production cutover, 2026-05-12, source commit `0381f00`, manifest digest `sha256:9182c9ef...`):*

- **`output_type.persist_fields`** — agent YAML can declare which schema-validated output slots to auto-persist across multi-turn calls; Nova OS injects a `## Known fields` system block on subsequent turns so the model doesn't re-ask. Strategies: `non_null_overwrite` (default), `deep_merge_non_null`, `append_unique`, `last_wins`. Scope keys: `session:{tenant}/{session}/{schema_id}` (default) or `end_user_agent:{tenant}/{api_key}/{end_user}/{schema_id}`. Active on `/agents/v1/.../chat` (Nova-native) and `/v1/chat/completions` (OAI-compat), both sync + streaming. Merged state surfaces as `metadata.persisted_state` on responses. PII-safe audit log on user-correction overwrites. New PG table `output_type_persisted_state` with memory fallback. (PRs [#314](https://github.com/MeganovaAI/nova-os/pull/314), #318, #323, #324; specs/issues [#313](https://github.com/MeganovaAI/nova-os/issues/313), #317, #319, #321, #322. Closes `nova-os#313`.)
- **Observability redesign** — `/conversations` card-tree replaced by an Activity-page kernel-inspector (flat sortable table, 8-tab right drawer, Mermaid call graph, restored brain canvas, brain-precision data layer). Old `/conversations/...` bookmark URLs redirect into the drawer-open form. Partners scraping or embedding the conversations UI should re-target the new shape. (PRs #304–#311, #316.)
- **`output_type.schema_inline` enforcement hardening** — four rounds of regression fixes pinned by wire-level tests; partners relying on schema-validated outputs should see fewer silent escapes through the validator. (PRs #292, #298, #300, #302.)
- **User-runtime agents directory** — new writable `data/agents/_runtime/` layer (gitignored, not baked into the release image) for runtime-authored personas. `NOVA_OS_AGENTS_RUNTIME_DIR` env override for production volume mounts. AgentsPage gains a preset/custom split. (PRs #315, #316.)
- **Smaller standalone binaries** — `nova-os-vX.Y.Z-*` dropped ~105 MB → ~57 MB after `web/static/{assets,index.html}` was git-untracked, the Dockerfile dashboard stage actually runs vite, and `release.yml` runs `make dashboard` before `go build`. Behaviour unchanged.
- **SSE wire-dump capture** — streaming turns now persist the assembled SSE wire bytes to a new `pipeline_events.sse_wire_dump` column. Partners debugging streaming behaviour can pull exact wire output from the DB without round-tripping to operators.
- **`output_type.persist_fields` on `/v1/messages`** — the partner-prefix path that `c.messages.create()` and `c.messages.stream()` both hit. Sync wires `metadata.persisted_state` into the response body; streaming carries `persisted_state` on the terminal `done` event. Closes the SDK-consumer coverage gap from the v0.1.7 stable cut. Sync via PR [#328](https://github.com/MeganovaAI/nova-os/pull/328) (issue [#326](https://github.com/MeganovaAI/nova-os/issues/326), still open for residual cleanup); streaming via PR [#334](https://github.com/MeganovaAI/nova-os/pull/334) (closes [#320](https://github.com/MeganovaAI/nova-os/issues/320), wire shape `message_stop.persisted_state` per Anthropic-SDK-compat probe). New end-to-end example: [`python/examples/18_custom_persona_persist_fields.py`](examples/18_custom_persona_persist_fields.py). **Available in `:v0.1.7-week-2026-05-26` and later;** the un-suffixed `:v0.1.7` GHCR image lags the git tag until `docker-publish.yml` rebuilds.
- **Cross-vendor Citations API + grounding passthrough** — partners can now post Anthropic-shape `document` content blocks via `c.messages.create()` and get structured citations back on response text blocks; the same shape comes back regardless of which provider answered. Anthropic provider emits `RequestDocumentBlock` + `CitationsConfigParam` and parses `TextCitationUnion` into `CitationRef`; Gemini provider auto-attaches `googleSearch` and aligns `GroundingSupports` into the same `Citations` shape; OpenAI-compat path emits `url_citation` annotations on `/v1/chat/completions`. Active across all three chat endpoints (`/v1/messages`, `/agents/v1/.../chat`, `/v1/chat/completions`), both sync + streaming. Main thread: PR [#346](https://github.com/MeganovaAI/nova-os/pull/346) (closes [#345](https://github.com/MeganovaAI/nova-os/issues/345)); per-endpoint rollout PRs [#348](https://github.com/MeganovaAI/nova-os/pull/348) (streaming `/v1/messages` `citations_delta`), [#349](https://github.com/MeganovaAI/nova-os/pull/349) (sync `/agents/v1/.../chat` with `cite_source` discriminator), [#350](https://github.com/MeganovaAI/nova-os/pull/350) (sync `/v1/chat/completions`), [#355](https://github.com/MeganovaAI/nova-os/pull/355) / [#356](https://github.com/MeganovaAI/nova-os/pull/356) / [#357](https://github.com/MeganovaAI/nova-os/pull/357) (streaming citations on the remaining endpoints). Closes the per-cell matrix (3 endpoints × sync/streaming = 6 cells, all ✅). [#358](https://github.com/MeganovaAI/nova-os/pull/358) (ProviderChain Citations routing) closed as not-planned per the revert thread. **Available in nova-os v0.1.7 round-7 force-refresh (commit `baab9364`, 2026-05-14)** — the un-suffixed `:v0.1.7` GHCR image still lags pending `docker-publish.yml`; pin to a `v0.1.7-week-2026-05-26` rebuild or a self-build off the tag for partner-validation work.
