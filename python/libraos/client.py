...
from libraos.errors import parse_error_response
...
class Client:
    ...
    def __init__(
        ...
        *,
        ...
        from libraos.resources.agents import Agents
        from libraos.resources.employees import Employees
        from libraos.resources.messages import Messages
        from libraos.resources.jobs import Jobs

        from libraos.resources.documents import Documents
        from libraos.resources.knowledge import Knowledge
        from libraos.resources.hooks import Hooks
        from libraos.resources.filesystem import Filesystem
        from libraos.resources.users import Users
        from libraos.resources.settings import Settings
        from libraos.resources.sessions import Sessions
        from libraos.resources.personas import Personas
        ...
        from libraos._sync import _SyncProxy
    ...
    def simulate(
        ...
        **kwargs: Any,
        ...
        See :func:`libraos.simulator.simulate` for the full signature
        ...
        :class:`~libraos.simulator.TurnEvent` — one event per turn
        plus a final ``outcome`` event. The outcome event always
        fires, even on error / timeout / cancellation paths.
        """
        if stream:
            from libraos.simulator.simulate import simulate_stream

            return simulate_stream(self, target_agent_id, archetype, **kwargs)

        from libraos.simulator.simulate import simulate as _simulate
    ...
    def async_simulate(
        ...
        **kwargs: Any,
        ...
        :class:`~libraos.simulator.SimulationResult` — ``await`` it.

        When ``stream=True``, returns an
        :class:`~typing.AsyncIterator` of
        :class:`~libraos.simulator.TurnEvent` — iterate with
        ...
        """
        if stream:
            from libraos.simulator.simulate import async_simulate_stream

            return async_simulate_stream(
                self, target_agent_id, archetype, **kwargs
            )

        from libraos.simulator.simulate import async_simulate as _async_simulate
